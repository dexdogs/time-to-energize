  ...
  return (
    <div ref={ref} style={{ position: 'absolute', bottom: '90px', right: '16px', zIndex: 30 }}>
      {open && (
        <div style={{ marginBottom: '8px', background: 'rgba(10,15,30,0.97)', border: '1px solid #1E2D50', borderRadius: '4px', backdropFilter: 'blur(12px)', minWidth: '210px' }}>
          <div style={{ padding: '8px 12px', borderBottom: '1px solid #1E2D50' }}>
            <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: '11px', color: '#6B7280', letterSpacing: '0.1em' }}>EXPORT DATASET</span>
          </div>
          {files.map(f => (
            <button key={f.label} onClick={() => downloadFile(f.label, f.path)}
              style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '100%', textAlign: 'left', padding: '8px 12px', fontFamily: "'Space Grotesk', sans-serif", fontSize: '11px', color: '#D1D5DB', background: 'transparent', border: 'none', borderBottom: '1px solid #111C35', cursor: 'pointer' }}
      ...
      <button onClick={() => setOpen(!open)}
        style={{
          fontFamily: "'Space Grotesk', sans-serif", fontSize: '11px', padding: '6px 12px', borderRadius: '4px',
